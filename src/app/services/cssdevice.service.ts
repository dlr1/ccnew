...
export class CSSDeviceService {
    ...
    constructor(private http: HttpClient, private reconService: ReconService, private ringLib: RingLib) {
    ...
    private async getCSSCiscoPanelDetails(ring, device, selectedRing, command, parser) {
        ...
        for (var i = 0; i < 2; i++) {
            ...
            if (data["error"] != null)
                ...
                var retValues = parser(command.keys, data["responses"][0].response, device.name);
    ...
    async getCSSAlcatelData(ring, device, selectedRing) {        
        ...
        Promise.all(promiseArray).then(function (allResponses) {
            ...
            if (data.error == null) {
                ...
                try {
                    var ports = parser(panel.commands[0].keys, data.responses[0].response);
                    ...
                    this.reconService.sendCommand({
                        ...
                        var parser = this.ringLib.getParser(panel.parser);
                        var data = parser(panel.commands[1].keys, responseData.responses[0].response);
    ...
    async getMetadata(ring, device, selectedRing) {
            ...
            if (data["error"] != null) {
                    ...
                    if (isCiscoCSS) {
                        ...
                        //pocService.getDisconnected({ session_key: device.sessionKey });
                        return 0;
